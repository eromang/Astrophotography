...
| Mel 111 first light | `05_Sessions/2026/Processing/2026-06-01-Processing.md` | **✅ Production master done 2026-06-01** — re-stacked with the genuine L-Pro flat, drizzle 2×, **4/4 astrometric solved offline**; full OpenCluster-Workflow as-executed. FWHM 4.36→**1.85 px**, ecc 0.37→**0.26**, **γ Com halo resolved** (was an uncorrected L-Pro filter reflection). New export is the vault result image. Residual field eccentricity (sensor tilt) still worth chasing at capture |
...
| L-Pro flat | `04_Processing/Calibration/Master-Library.md` | **✅ Done 2026-05-31** — first genuine L-Pro flat shot + built (50 × 10 ms → `Masters/Flat/LPro/`); calibration library complete. **Re-stack of Mel 111 with it: done 2026-06-01** (γ Com halo fixed). Master flat XISF `Instrument:Filter:Name` property set to `LPro` (WBPP reads the property, not the FITS keyword); FQuad masters patched too — `set_filter.py` XISF mode |
